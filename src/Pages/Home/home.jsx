...
import ScrollableCarousel from "../../Components/HorizontalScroll/index";
                ...
                >
                    {
                        popularMovies.map((movie,index) => (
                            <Link style={{textDecoration:"none",color:"white"}} to={movie.release_date?`/movie/${movie.id}`:`/tvshow/${movie.id}`} key={index} >
                                <div className="posterImage bg-gray-900 h-[600px] max-lg:h-[450px] max-md:h-[350px] max-bs:h-[300px] max-sm:h-[250px] max-xs:h-52">
                                    <img src={`https://image.tmdb.org/t/p/original${movie && movie.backdrop_path}`} className="m-auto block w-full"/>
                                </div>
                                <div className="posterImage-overlay absolute p-20 bottom-0 h-7/10 flex flex-col w-full justify-end items-start opacity-100 transition-opacity duration-300 bg-gradient-to-t from-[#111827] to-transparent hover:opacity-100 max-lg:p-12 max-lg:h-3/5 max-md:p-8 max-md:h-1/2 max-bs:p-8 max-sm:p-4 max-xs:p-2 max-xs:h-2/5 max-bs:pb-16 max-sm:pt-36 max-md:pb-18">
                                    <div className="posterImage__title font-black text-6xl mb-3 text-left max-lg:text-5xl max-md:text-4xl max-bs:text-3xl max-xs:text-xl max-sm:text-2xl">{movie ? movie.original_title || movie.original_name: ""}</div>
                                    ...
                                    <div className="posterImage__description italic text-base mb-1 flex text-left w-1/2 max-md:text-sm max-lg:w-3/5 max-md:w-[70%] max-bs:text-xs max-bs:w-4/5 max-sm:hidden">{movie ? movie.overview.slice(0,200)+"..." : ""}</div>
                                </div>
                            </Link>
                        ))
                    }
